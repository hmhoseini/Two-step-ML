...
results = mpr.query(criteria={'nelements':2, 'is_hubbard':False, 'is_compatible': True},\
                    properties=['pretty_formula', 'formation_energy_per_atom', 'material_id', 'e_above_hull', 'is_hubbard', 'is_compatible']
                    )
with open('allBinaries_list.dat', 'w') as fhandle:
    for a_result in results:
        fhandle.write(a_result['pretty_formula']+'\n')
with open('allBinaries_dict.dat', 'w') as fhandle:
